...
SECTION(".init.text") ACPI::ACPI(TableHeader* header, unsigned int rev)
{
	...
	console().WriteMessage(Console::MSG_INFO, "ACPI %4s:", "OEMID: %6s, OEM Table ID: %8s, OEM Rev.: %d", (header->Signature).Bytes, header->OEMID, header->OEMTabID, header->OEMRev);
	console().WriteMessage(Console::MSG_INFO, "ACPI %4s:", "Creator ID: %4s, Creator Rev.: %d", (header->Signature).Bytes, header->CreatorID, header->CreatorRev);
...
{
	console().WriteMessage(Console::MSG_INFO, "Hardware Signature:", "0x%8x", facs->HardwareSignature);
}
...
void SECTION(".init.text") ACPI::ParseMadt(MadTable* madt)
{
	...
	for(ptr = (unsigned char*)((unsigned long)madt + sizeof(MadTable)); (unsigned long)ptr < (unsigned long)madt + (madt->Header).Length; ptr += ptr[1])
	{
		...
		case ENTRY_LAPIC:
			...
			console().WriteMessage(Console::MSG_INFO, "CPU:", "CPU %d, LAPIC ID %d", cpus[numcpus]->ProcessorID, cpus[numcpus]->ApicID);
			numcpus++;
			break;
		case ENTRY_IOAPIC:
			ioapics[numioapics] = (IOApic*)ptr;
			console().WriteMessage(Console::MSG_INFO, "I/O APIC:", "#%d (IRQ base %d) @ 0x%8x", ioapics[numioapics]->ApicID, ioapics[numioapics]->InterruptBase, ioapics[numioapics]->Address);
			numioapics++;
			break;
		case ENTRY_IRQ_SRC:
			irqsrcs[numirqsrcs] = (IrqSourceOverride*)ptr;
			console().WriteMessage(Console::MSG_INFO, "IRQ SRC:", "Bus #%d IRQ #%d => INT #%d", irqsrcs[numirqsrcs]->Bus, irqsrcs[numirqsrcs]->Source, irqsrcs[numirqsrcs]->Interrupt);
			numirqsrcs++;
			break;
		case ENTRY_NMI:
			nmisrcs[numnmisrcs] = (NMI*)ptr;
			console().WriteMessage(Console::MSG_INFO, "NMI:", "INT #%d", nmisrcs[numnmisrcs]->Interrupt);
			numnmisrcs++;
			break;
		case ENTRY_LAPIC_NMI:
			lapicnmis[numlapicnmis] = (LApicNMI*)ptr;
			console().WriteMessage(Console::MSG_INFO, "LAPIC NMI:", "CPU #%d, LINT #%d", lapicnmis[numlapicnmis]->ProcessorID, lapicnmis[numlapicnmis]->LINT);
			numlapicnmis++;
			break;
		case ENTRY_LAPIC_ADDR:
			lapicaddrs[numlapicaddrs] = (LApicAddressOverride*)ptr;
			console().WriteMessage(Console::MSG_INFO, "LAPIC Addr:", "Override to 0x%16lx", lapicaddrs[numlapicaddrs]->Address);
			numlapicaddrs++;
			break;
		case ENTRY_IOSAPIC:
			iosapics[numiosapics] = (IOSApic*)ptr;
			console().WriteMessage(Console::MSG_INFO, "I/O SAPIC:", "#%d (IRQ base %d) @ 0x%16lx", iosapics[numiosapics]->ApicID, iosapics[numiosapics]->InterruptBase, iosapics[numiosapics]->Address);
			numiosapics++;
			break;
		case ENTRY_LSAPIC:
			lsapics[numlsapics] = (LSApic*)ptr;
			console().WriteMessage(Console::MSG_INFO, "LSAPIC:", "CPU #%d, LSAPIC ID %d, LSAPIC EID %d", lsapics[numlsapics]->ProcessorID, lsapics[numlsapics]->LSApicID, lsapics[numlsapics]->LSApicEID);
			numlsapics++;
			break;
		case ENTRY_PLATFORM:
			platirqs[numplatirqs] = (PlatformInterrupt*)ptr;
			console().WriteMessage(Console::MSG_INFO, "PMI:", "Dest ID %d, Dest EID %d, Vector %d, INT #%d", platirqs[numplatirqs]->ProcessorID, platirqs[numplatirqs]->ProcessorEID, platirqs[numplatirqs]->Vector, platirqs[numplatirqs]->Interrupt);
			numplatirqs++;
			break;
		case ENTRY_X2APIC:
			x2apics[numx2apics] = (X2Apic*)ptr;
			console().WriteMessage(Console::MSG_INFO, "X2APIC:", "X2APIC ID %d, CPU %4s", x2apics[numx2apics]->X2ApicID, &(x2apics[numx2apics]->AcpiID));
			numx2apics++;
			break;
		case ENTRY_X2APIC_NMI:
			x2apicnmis[numx2apicnmis] = (X2ApicNMI*)ptr;
			console().WriteMessage(Console::MSG_INFO, "X2APIC NMI:", "CPU %4s, LINT #%d", &(x2apicnmis[numx2apicnmis]->AcpiID), x2apicnmis[numx2apicnmis]->LINT);
			numx2apicnmis++;
			break;
		default:
			break;
...
{
	console().WriteMessage(Console::MSG_INFO, "HPET # %d:", "%d %d-bit counters found at 0x%8x", hpet->Number, (hpet->EventTimerBlockID).Data.ComparatorCount + 1, ((hpet->EventTimerBlockID).Data.CountSize64Bit ? 64 : 32), (hpet->BaseAddress).Address);
...
void SECTION(".init.text") ACPI::ParseTable(unsigned long phys)
{
	...
	if(checksum > 0)
	{
		console().WriteMessage(Console::MSG_WARNING, "ACPI %4s:", "checksum failed at 0x%8x, ignored", (header->Signature).Bytes, phys);
		return;
	}

	console().WriteMessage(Console::MSG_OK, "ACPI %4s:", "found at 0x%8x", (header->Signature).Bytes, phys);
...
bool SECTION(".init.text") ACPI::Init(unsigned long first, unsigned long last)
{
	...
	{
		pointer = (RsdPtr*)(phys + Memory::KernelOffset);
			...
			(pointer->Signature[4] == 'P') && (pointer->Signature[5] == 'T') && (pointer->Signature[6] == 'R') && (pointer->Signature[7] == ' '))
		{
			...
			if(checksum > 0)
			{
				console().WriteMessage(Console::MSG_WARNING, "ACPI RSD pointer:", "1st checksum failed at 0x%8x, ignored", phys);
				continue;
			}
			if(pointer->Revision > 0)
			{
				...
				if(checksum > 0)
				{
					console().WriteMessage(Console::MSG_WARNING, "ACPI RSD pointer:", "2nd checksum failed at 0x%8x, ignored", phys);
					continue;
				}
			}
			console().WriteMessage(Console::MSG_OK, "ACPI RSD pointer:", "Rev. %d found at 0x%8x", pointer->Revision, phys);
			tab = ((pointer->Revision == 0) ? pointer->RsdtAddress : pointer->XsdtAddress);
			header = (TableHeader*)x86pager().PhysToVirt(tab);
			console().WriteMessage(Console::MSG_OK, "ACPI %4s:", "found at 0x%8x", (header->Signature).Bytes, tab);
			checksum = 0;
			csptr = (unsigned char*)header;
			for(i = 0; i < header->Length; i++)
				checksum += csptr[i];
			if(checksum > 0)
			{
				console().WriteMessage(Console::MSG_WARNING, "ACPI %4s:", "checksum failed, ignored", (header->Signature).Bytes);
				continue;
			}
	...
	console().WriteMessage(Console::MSG_INFO, "ACPI RSD pointer:", "not found in range 0x%8x to 0x%8x", first, last);
	return(false);
}
