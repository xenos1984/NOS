...
SECTION(".init.text") Console::Console(void)
{
	// Set message colors
	infoback = msgback[MSG_INFO] = msgback[MSG_OK] = msgback[MSG_WARNING] = msgback[MSG_ERROR] = BG_BLACK;
	infofore = FG_LIGHT_GRAY;
	msgfore[MSG_INFO] = FG_WHITE;
	msgfore[MSG_OK] = FG_GREEN;
	msgfore[MSG_WARNING] = FG_YELLOW;
	msgfore[MSG_ERROR] = FG_RED;
}
...
template <class T> void Console::itoa(char *buf, T divisor, T number, int length)
{
	char *p = buf;
	char *p1, *p2;
	char tmp;
...
void Console::writef(const char* format, va_list* arg)
{
	char c;
	const char *p;
	char buf[24];
...
void Console::WriteMessage(uint8_t type, const char *message, const char *result, ...)
{
	va_list arg;
	uint8_t bg, fg;
	...
	bg = GetBackground();
	fg = GetForeground();
	SetBackground(infoback);
	SetForeground(infofore);
	writef(message, &arg);
	Write('\t');
	SetBackground(msgback[type]);
	SetForeground(msgfore[type]);
	writef(result, &arg);
	Write('\n');
	SetBackground(bg);
	SetForeground(fg);
